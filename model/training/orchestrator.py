...
class Orchestrator:
    ...
    def run_transfer_learning_pipeline(self):
        ...
        self.transfer_learning_history, tl_best_epoch, tl_last_epoch = (
        ...
        if self.config.get("fine_tune", False):
            ...
            self.model = self.learning_configurator.prepare_model_for_fine_tuning(
                self.model, self.config["num_unfreezed_feature_layers"]
            )
            ...
            self.fine_tuning_history, ft_best_epoch, ft_last_epoch = (
                self.trainer.train_model(
                    self.model,
        ...
        final_history = (
            {
                key: self.transfer_learning_history[key] + self.fine_tuning_history[key]
                for key in self.transfer_learning_history
            }
            if self.fine_tuning_history
            else self.transfer_learning_history
        )
        self.best_epoch = (
            tl_last_epoch + self.best_epoch
            if self.fine_tuning_history
            else tl_best_epoch
        )
        self.last_epoch = (
            tl_last_epoch + ft_last_epoch if self.fine_tuning_history else tl_last_epoch
        )
        ...
        best_model_weights_path = os.path.join(
            self.output_path,
            (
                f"{self.model_name}_fine_tuning"
                if self.fine_tuning_history
                else f"{self.model_name}_transfer_learning"
            )
            + ".pt",
        )
        ...
        self.evaluator.save_test_metrics(
            best_train_acc,
            best_val_acc,
    ...
    def run_transfer_learning_only(self, model=None):
        ...
        self.model = self.learning_configurator.prepare_model_for_transfer_learning(self.model)
        self.transfer_learning_history, self.best_epoch, self.last_epoch = (
            self.trainer.train_model(
                self.model,
                epochs=self.config["transfer_learning_epochs"],
                learning_rate=self.config["transfer_learning_rate"],
    ...
    def run_fine_tuning_only(self, load_transfer_learning_checkpoint=True, model=None):
        ...
        if load_transfer_learning_checkpoint:
            ...
            self.model_factory.load_checkpoint(
                self.model, os.path.join(self.output_path, f"{self.model_name}_transfer_learning.pt")
            )
        ...
        # Train with fine-tuning
        self.fine_tuning_history, self.best_epoch, self.last_epoch = (
            self.trainer.train_model(
                self.model,
                epochs=self.config["fine_tune_epochs"],
                learning_rate=self.config["fine_tuning_learning_rate"],
    ...
    def evaluate_only(self, model_path=None, model=None):
        ...
        recall = evaluation_results["recall"]
        f1 = evaluation_results["f1_score"]
        self.evaluator.save_test_metrics(0, 0, evaluation_results["test_accuracy"], precision, recall, f1, "best-")
        self.evaluator.plot_confusion_matrix(evaluation_results["confusion_matrix"], self.dataset_handler.get_class_names())
        self.evaluator.generate_evaluation_report(evaluation_results)
    ...
    def get_component_info(self):
        ...
        info = {
            "dataset_handler": {
                ...
                "device": self.dataset_handler.get_device(),
                "train_size": (
                    len(self.dataset_handler.train_data)
                    if self.dataset_handler.train_data
                    else 0
                ),
                "val_size": (
                    len(self.dataset_handler.val_data)
                    if self.dataset_handler.val_data
                    else 0
                ),
                "test_size": (
                    len(self.dataset_handler.test_data)
                    if self.dataset_handler.test_data
                    else 0
                ),
            },
            "model_factory": {
