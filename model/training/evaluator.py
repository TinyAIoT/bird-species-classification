...
class Evaluator:
    ...
    def evaluate_model(self, model, weigths_path=None):
        ...
        print(f"Class names in evaluator: {class_names}")
        cm = confusion_matrix(all_labels, all_preds, labels=range(len(class_names)))

        # Calculate precision, recall, F1 score and accuracy
        precision, recall, f1 = self._calculate_precision_recall_f1(cm)
    ...
    def _calculate_precision_recall_f1(self, cm, averaging="micro"):
        ...
        # for micro precision and recall
        total_tp = 0
        total_fp = 0
        total_fn = 0
        ...
        print(f"Precision, recall and f1 results: {results}")
        if averaging == "macro":
            print("Using macro averaging for precision, recall and f1. \n")
            return macro_precision, macro_recall, macro_f1
        else:
            print("Using micro averaging for precision, recall and f1. \n")
    ...
    def save_test_metrics(self, train_accuracy, val_accuracy, test_accuracy, precision, recall, f1, prefix=""):
        ...
        """
        test_metrics = np.array([[train_accuracy, val_accuracy, test_accuracy, precision, recall, f1]])
        np.savetxt(
            os.path.join(self.output_path, self.model_name + "_" + prefix + "metrics.csv"),
            test_metrics,
            delimiter=",",
            header="best_train_accuracy,best_val_accuracy,test_accuracy,precision,recall,f1",
            comments="",
            fmt="%f",
        )
